<x-app-layout>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5">
                        Total <span class="font-bold text-brand-600 dark:text-brand-400">{{ $archives->total() }}</span>
                        dokumen tersimpan
                    </p>
                </div>
            </div>
    ...
    <div class="py-12" x-data="{ showDeleteModal: false, deleteAction: null }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div
                    class="mb-8 flex items-center p-4 bg-emerald-50/80 dark:bg-emerald-900/20 backdrop-blur-sm border border-emerald-200 dark:border-emerald-800 rounded-2xl text-emerald-800 dark:text-emerald-200 shadow-sm animate-in fade-in slide-in-from-top-2 duration-500">
            ...
            @if ($archives->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    @foreach ($archives as $archive)
                        <div
                            class="group relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm hover:shadow-xl hover:shadow-brand-500/5 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full overflow-hidden">

                            {{-- Card Header --}}
                            <div class="p-5 pb-0 flex items-start justify-between relative z-10">
                                <div
                                    class="p-2.5 bg-brand-50 dark:bg-brand-900/20 rounded-xl text-brand-600 dark:text-brand-400 group-hover:bg-brand-600 group-hover:text-white transition-colors duration-300">
                                ...
                                <span
                                    class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-600">
                                    #{{ $archive->id }}
                                </span>
                            </div>
                            ...
                            <div class="p-5 flex-grow flex flex-col">
                                <h3 class="font-bold text-lg text-gray-800 dark:text-gray-100 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors line-clamp-2 mb-3"
                                    title="{{ $archive->title }}">
                                    {{ $archive->title }}
                                </h3>

                                <div
                                    class="mt-auto space-y-2 pt-4 border-t border-gray-50 dark:border-gray-700/50 border-dashed">
                                    @if ($archive->creator)
                                        <div class="flex items-center text-xs font-medium text-gray-500 dark:text-gray-400">
                                            <svg class="w-3.5 h-3.5 me-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            {{-- Card Footer (Actions) --}}
                            <div
                                class="p-4 bg-gray-50/50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex gap-2">
                                <a href="{{ route('admin.archive.show', $archive->id) }}"
                                    class="flex-1 inline-flex items-center justify-center px-3 py-2 text-xs font-bold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-brand-50 hover:text-brand-600 hover:border-brand-200 transition-all duration-200">
                                    Lihat
                                </a>
                                <a href="{{ route('admin.archive.edit', $archive) }}"
                                    class="inline-flex items-center justify-center px-3 py-2 text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/30 rounded-lg hover:bg-amber-100 hover:border-amber-200 transition-all duration-200">
                                    Edit
                                </a>
                                <button type="button"
                                    @click="deleteAction = '{{ route('admin.archive.destroy', $archive) }}'; showDeleteModal = true"
                                    class="p-2 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/30 rounded-lg hover:bg-red-600 hover:text-white hover:border-red-600 transition-all duration-200">
            ...
            @else
                {{-- Empty State (Modern) --}}
                <div
                    class="bg-white dark:bg-gray-800 overflow-hidden rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700 relative">
                    <div
                        class="absolute top-0 left-0 w-full h-full bg-grid-slate-100 dark:bg-grid-slate-700/25 [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0.6))] dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]">
                    </div>
                    <div class="relative p-16 flex flex-col items-center text-center z-10">
                        <div
                            class="w-24 h-24 bg-brand-50 dark:bg-gray-700 rounded-full flex items-center justify-center mb-6 shadow-inner">
                            <svg class="w-12 h-12 text-brand-200 dark:text-gray-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 mt-2 max-w-sm leading-relaxed">
                            Database arsip saat ini masih kosong. Silakan unggah dokumen pertama Anda untuk memulai
                            manajemen arsip.
                        </p>
                        <a href="{{ route('admin.archive.create') }}"
                            class="mt-8 px-8 py-3 bg-brand-600 text-white font-bold rounded-xl hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/30 hover:-translate-y-1">
                                ...
                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            Apakah Anda yakin ingin menghapus arsip ini? Tindakan ini tidak dapat
                                            dibatalkan dan semua data terkait akan hilang permanen.
                                        </p>
                                    </div>
                                </div>
                            </div>
