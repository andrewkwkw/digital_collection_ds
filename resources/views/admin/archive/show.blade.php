        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between w-full gap-6">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5 truncate max-w-xs md:max-w-md">
                        {{ $archive->title }}
                    </p>
                </div>
            </div>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-2">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    ...
                    {{-- Hapus --}}
                    <form action="{{ route('admin.archive.destroy', $archive) }}" method="POST" class="w-full md:w-auto" onsubmit="return confirm('PERINGATAN: Menghapus arsip ini akan menghapus seluruh file lampiran di dalamnya secara permanen.\n\nApakah Anda yakin ingin melanjutkan?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
                                class="inline-flex items-center justify-center w-full px-4 py-2 bg-red-600 hover:bg-red-500 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest shadow-sm transition ease-in-out duration-150">
                            <svg class="w-4 h-4 me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if ($archive->files->count() > 0)
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex items-center justify-between">
                        ...
                        <span class="text-xs font-medium text-gray-500 bg-white dark:bg-gray-700 px-2 py-1 rounded-md border border-gray-200 dark:border-gray-600">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 gap-4">
                            @foreach ($archive->files as $file)
                                <div class="group flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-brand-500 dark:hover:border-brand-500 hover:shadow-md hover:shadow-brand-500/5 transition-all duration-300">
                                    <div class="flex items-center gap-4 overflow-hidden">
                                        <div class="flex-shrink-0 w-12 h-12 bg-red-50 dark:bg-red-900/20 rounded-lg flex items-center justify-center border border-red-100 dark:border-red-800/30">
                                            <svg class="w-6 h-6 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                            </svg>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-sm font-bold text-gray-800 dark:text-gray-200 truncate group-hover:text-brand-600 transition-colors">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 lg:p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label class="text-[10px] uppercase tracking-widest font-bold text-gray-400 dark:text-gray-500 mb-1 block">Judul Dokumen</label>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    <label class="text-[10px] uppercase tracking-widest font-bold text-gray-400 dark:text-gray-500 mb-1 block">Nomor/Tanggal</label>
                                    <p class="font-medium text-gray-700 dark:text-gray-300">
                                        {{ $archive->date ? \Carbon\Carbon::parse($archive->date)->format('d F Y') : '-' }}
                                    </p>
                                </div>
                                <div>
                                    <label class="text-[10px] uppercase tracking-widest font-bold text-gray-400 dark:text-gray-500 mb-1 block">Pencipta</label>
                                    <p class="font-medium text-gray-700 dark:text-gray-300">
                                        {{ $archive->creator ?? '-' }}
                                    </p>
                                </div>
                            ...
                            <div>
                                <label class="text-[10px] uppercase tracking-widest font-bold text-gray-400 dark:text-gray-500 mb-1 block">Deskripsi</label>
                                <div class="prose prose-sm dark:prose-invert max-w-none text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700/50 italic">
                        ...
                        <div>
                            <div class="bg-gray-50 dark:bg-gray-700/20 rounded-xl border border-gray-100 dark:border-gray-700/50 overflow-hidden">
                                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700/50">
                                    <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wide">Metadata Teknis</h4>
                                </div>
                                <div class="divide-y divide-gray-100 dark:divide-gray-700/50">
                                    ...
                                    @foreach($metaRows as $row)
                                        <div class="grid grid-cols-3 gap-4 px-4 py-3 text-sm hover:bg-white dark:hover:bg-gray-700/30 transition-colors">
                                            <div class="font-medium text-gray-500 dark:text-gray-400">{{ $row['label'] }}</div>
                                            <div class="col-span-2 font-semibold text-gray-800 dark:text-gray-200 break-words">
                                                {{ $row['value'] ?? '-' }}
                                            </div>
                                        </div>
                                    @endforeach
