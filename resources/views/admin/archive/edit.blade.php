    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div
                ...
                <div class="p-8">
                    <form method="POST" action="{{ route('admin.archive.update', $archive) }}"
                        ...
                        @if ($archive->files->count() > 0)
                            <div>
                                <div class="flex items-center gap-2 mb-4">
                                    <h3 class="text-xs font-black uppercase tracking-[0.2em] text-gray-400">
                                        {{ __('Koleksi PDF Saat Ini') }}</h3>
                                ...
                                <div class="grid grid-cols-1 gap-3">
                                    @foreach ($archive->files as $file)
                                        <div
                                            class="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-gray-900/40 rounded-2xl border border-gray-100 dark:border-gray-700 group hover:border-brand-200 transition-colors">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="w-10 h-10 flex items-center justify-center bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                                                ...
                                                <div class="flex flex-col">
                                                    ...
                                                    <span
                                                        class="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">Klik
                                                        untuk meninjau file</span>
                                                </div>
                                            </div>
                                            <button type="button"
                        ...
                        <div
                            class="bg-brand-50/30 dark:bg-brand-500/5 p-6 rounded-2xl border border-dashed border-brand-200 dark:border-brand-500/20">
                            <label for="documents"
                                class="block text-sm font-black text-brand-700 dark:text-brand-400 uppercase tracking-widest mb-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                <div class="md:col-span-2">
                                    ...
                                    <x-text-input id="title"
                                        class="block mt-1 w-full bg-gray-50/50 dark:bg-gray-900/40 border-gray-200 focus:border-brand-500 focus:ring-brand-500 rounded-xl"
                                        type="text" name="title" :value="old('title', $archive->title)" required />
                                    <x-input-error :messages="$errors->get('title')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="creator" :value="__('Pembuat (Creator)')" class="font-bold" />
                                    <x-text-input id="creator"
                                        class="block mt-1 w-full bg-gray-50/50 dark:bg-gray-900/40 border-gray-200 focus:border-brand-500 focus:ring-brand-500 rounded-xl"
                                        type="text" name="creator" :value="old('creator', $archive->creator)" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="date"
                                        class="block mt-1 w-full bg-gray-50/50 dark:bg-gray-900/40 border-gray-200 focus:border-brand-500 focus:ring-brand-500 rounded-xl"
                                        type="date" name="date" :value="old('date', $archive->date?->format('Y-m-d'))" />
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="subject" name="subject" rows="2"
                                        class="block mt-1 w-full bg-gray-50/50 dark:bg-gray-900/40 border-gray-200 dark:border-gray-700 dark:text-gray-100 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm transition-all">{{ old('subject', $archive->subject) }}</textarea>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="description" name="description" rows="3"
                                        class="block mt-1 w-full bg-gray-50/50 dark:bg-gray-900/40 border-gray-200 dark:border-gray-700 dark:text-gray-100 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm transition-all">{{ old('description', $archive->description) }}</textarea>
                                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                                </div>

                                <div
                                    class="md:col-span-2 grid grid-cols-2 lg:grid-cols-3 gap-4 mt-4 p-5 bg-gray-50/30 dark:bg-gray-900/10 rounded-2xl border border-gray-100 dark:border-gray-700">
                                    @php
                                        $fields = [
                                            'publisher' => 'Penerbit',
                                            'type' => 'Tipe',
                                    ...
                                    @foreach($fields as $id => $label)
                                        <div>
                                            <x-input-label for="{{ $id }}" :value="__($label)"
                                                class="text-[10px] uppercase font-black text-gray-400" />
                                            <x-text-input id="{{ $id }}"
                                                class="block mt-1 w-full text-sm border-gray-100 dark:bg-gray-800 rounded-lg"
                                                type="text" name="{{ $id }}" :value="old($id, $archive->$id)" />
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div
                            class="flex items-center justify-between mt-12 pt-8 border-t border-gray-100 dark:border-gray-700">
                            <a href="{{ route('admin.archive.show', $archive) }}"
                                class="inline-flex items-center text-sm font-bold text-gray-400 hover:text-gray-600 transition-colors">
                            ...
                            <x-primary-button
                                class="bg-brand-500 hover:bg-brand-600 px-10 py-3.5 rounded-2xl shadow-xl shadow-brand-500/20 transform transition active:scale-95">
                                {{ __('Perbarui & Simpan') }}
                            </x-primary-button>
                        </div>
                    </form>
