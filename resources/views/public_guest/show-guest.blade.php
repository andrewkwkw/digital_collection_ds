...
<head>
    ...
    <title>{{ $archive->title }} - {{ config('app.name', 'Laravel') }}</title>
            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-6 text-white">
                        ...
                        <p class="text-blue-100">{{ __('Diunggah pada') }} {{ $archive->created_at->format('d F Y, H:i') }}</p>
                    </div>
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        @if ($archive->files->count() > 0)
                            <div class="mb-8">
                                <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">{{ __('File PDF') }}</h3>
                                <div class="space-y-3">
                                    @foreach ($archive->files as $file)
                                        <div
                                            ...
                                            <div class="flex-1">
                                                <p class="font-semibold text-gray-900 dark:text-white mb-1">{{ $file->original_filename ?? basename($file->archive_path) }}</p>
                        ...
                        @else
                            <div class="mb-8 p-6 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                                <p class="text-yellow-800 dark:text-yellow-200">{{ __('Tidak ada file yang tersedia untuk arsip ini.') }}</p>
                            </div>
                        @endif
