...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700|playfair-display:400,600,700,800"
        rel="stylesheet" />
...
<body
    class="bg-gray-50 dark:bg-brand-25 antialiased font-['Instrument_Sans'] text-gray-900 dark:text-gray-100 flex flex-col min-h-screen relative">
    ...
    <main class="flex-grow py-10 px-4 sm:px-6 lg:px-8 relative">
        <div class="max-w-7xl mx-auto">
            ...
            <div
                class="bg-white dark:bg-brand-25 rounded-[2rem] shadow-xl overflow-hidden border border-brand-200 dark:border-brand-800">
                ...
                <div class="flex flex-col lg:flex-row">
                    ...
                    <div
                        class="w-full lg:w-[400px] bg-brand-50 dark:bg-brand-950/50 p-8 lg:p-10 border-r border-brand-100 dark:border-brand-800 flex flex-col items-center sticky top-0">

                        <div
                            class="relative w-full aspect-[210/297] max-w-[300px] mb-8 group flex flex-col items-center justify-center transition-transform hover:-translate-y-1 duration-500">

                            @if($archive->files->count() > 0)
                                {{-- Canvas Container --}}
                                <div
                                    class="w-full h-full bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 document-shadow overflow-hidden relative">

                                    {{-- Loading State --}}
                                    <div id="pdf-loading"
                                        class="absolute inset-0 flex items-center justify-center bg-brand-50 dark:bg-brand-900 z-10">
                                    ...
                                    {{-- The Actual Canvas --}}
                                    <canvas id="the-canvas" class="w-full h-full object-cover"
                                        data-url="{{ asset('storage/' . $archive->files->first()->archive_path) }}"></canvas>
                                </div>
                            @else
                                {{-- Fallback --}}
                                <div
                                    class="w-full h-full bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 document-shadow flex flex-col items-center justify-center text-gray-400">
                                    <svg class="w-16 h-16 mb-2 text-brand-300" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                        ...
                        <div class="w-full max-w-[300px] space-y-3">
                            ...
                            @else
                                <button disabled
                                    class="w-full flex items-center justify-center gap-2 bg-brand-100 text-brand-400 px-6 py-4 rounded-xl font-semibold cursor-not-allowed">
                                    File Tidak Tersedia
                                </button>
                            @endif
                        </div>
                    ...
                    <div class="flex-1 p-8 lg:p-12">
                        <div class="mb-10 border-b border-brand-100 dark:border-brand-800 pb-8">
                            <div class="flex items-center gap-3 mb-4">
                                <span
                                    class="px-3 py-1 rounded-md bg-brand-50 dark:bg-brand-900/50 text-brand-700 dark:text-brand-200 text-xs font-bold uppercase tracking-widest border border-brand-200 dark:border-brand-700">
                                    Arsip Publik
                                </span>
                            ...
                            <h1
                                class="text-3xl lg:text-5xl font-bold font-['Playfair_Display'] text-brand-900 dark:text-brand-50 leading-tight mb-4">
                                {{ $archive->title }}
                            </h1>
                        </div>
                        ...
                        <div class="space-y-8">
                            <div
                                class="bg-white dark:bg-brand-25 rounded-2xl border border-brand-200 dark:border-brand-700 p-1">
                                <div class="bg-brand-50/50 dark:bg-brand-950/30 rounded-xl p-6 sm:p-8">
                                    <h3
                                        class="flex items-center gap-2 text-lg font-bold text-brand-800 dark:text-brand-100 mb-6">
                            ...
                            @if ($archive->files->count() > 1)
                                <div>
                                    <h3 class="text-sm font-bold uppercase tracking-wider text-brand-400 mb-4">Lampiran
                                        Tambahan</h3>
                                    <div class="grid gap-3">
                                        @foreach ($archive->files->skip(1) as $file)
                                            <a href="{{ route('archive.show_file', $file->id) }}"
                                                class="flex items-center justify-between p-4 bg-white dark:bg-brand-900 border border-brand-200 dark:border-brand-700 rounded-lg hover:border-brand-500 hover:ring-1 hover:ring-brand-500 transition-all group">
                                                <div class="flex items-center gap-3">
                                                    {{-- Icon PDF merah agar kontras --}}
                                                    <svg class="w-8 h-8 text-red-600/80" fill="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path
                                                            d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15v-4H8l4-4 4 4h-3v4h-2z" />
                                                    </svg>
                                                    <p
                                                        class="font-medium text-gray-900 dark:text-white text-sm group-hover:text-brand-700 dark:group-hover:text-brand-300 transition-colors">
                                                        {{ $file->original_filename ?? basename($file->archive_path) }}
                                                    </p>
                                                </div>
                                                <svg class="w-5 h-5 text-brand-300 group-hover:text-brand-600" fill="none"
