...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700|playfair-display:400,600"
        rel="stylesheet" />
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-10 text-center sm:text-left">
                ...
                <p class="text-gray-600 dark:text-gray-400 max-w-2xl">
                    Jelajahi arsip digital kami. Gunakan fitur filter dan pencarian untuk menemukan referensi akademik
                ...
                @if($search || $filter)
                    <div
                        class="mt-6 inline-flex items-center px-4 py-2 rounded-lg bg-brand-100 dark:bg-brand-900/50 border border-brand-200 dark:border-brand-700 text-brand-800 dark:text-brand-200 text-sm">
                        ...
                        <a href="{{ route('jelajah') }}"
                            class="ml-3 text-brand-600 hover:text-brand-800 dark:hover:text-brand-400 font-semibold underline">Reset</a>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
                    ...
                    @foreach ($archives as $archive)
                        <a href="{{ route('archive.show-guest', $archive->id) }}"
                            ...
                            {{-- THUMBNAIL AREA --}}
                            <div
                                class="aspect-[16/9] w-full bg-brand-50 dark:bg-brand-950 relative border-b border-brand-100 dark:border-brand-800 overflow-hidden">

                                @if ($archive->files->count())
                                    <div class="pdf-thumb w-full h-full relative"
                                        data-pdf="{{ asset('storage/' . $archive->files->first()->archive_path) }}">
                                        {{-- Loading Spinner (Hilang setelah render) --}}
                                        <div
                                            class="loading-spinner absolute inset-0 flex items-center justify-center bg-brand-50 dark:bg-brand-900 z-10">
                                ...
                                @else
                                    {{-- Fallback No Preview --}}
                                    <div
                                        class="w-full h-full flex flex-col items-center justify-center text-brand-300 dark:text-brand-700 p-4 text-center">
                                ...
                                <div
                                    ...
                                    <span
                                        class="px-4 py-2 bg-white/90 dark:bg-brand-950/90 text-brand-900 dark:text-brand-100 text-xs font-bold rounded-full shadow-lg backdrop-blur-sm transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                        Lihat Detail
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="p-5 flex flex-col flex-grow">
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    <span
                                        class="px-2 py-1 rounded bg-brand-50 dark:bg-brand-950 border border-brand-100 dark:border-brand-700 text-[10px] font-bold uppercase tracking-wider text-brand-600 dark:text-brand-400">
                                ...
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white leading-snug mb-2 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors line-clamp-2"
                                    title="{{ $archive->title }}">
                                    {{ $archive->title }}
                                </h3>

                                <div
                                    class="mt-auto pt-4 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-brand-800">
            ...
            @else
                {{-- EMPTY STATE --}}
                <div
                    class="flex flex-col items-center justify-center py-20 bg-white dark:bg-brand-900 rounded-3xl border border-brand-100 dark:border-brand-800 text-center shadow-sm">
                    ...
                    <a href="{{ route('jelajah') }}"
                        class="mt-6 px-6 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-lg font-medium transition-colors">
                        Reset Pencarian
                    </a>
                </div>
            @endif
