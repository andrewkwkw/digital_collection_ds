...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600,700|playfair-display:400,600,700"
        rel="stylesheet" />
...
<body
    ...
    <main class="flex-grow">
        ...
        {{-- ================= HERO SECTION ================= --}}
        <div
            class="relative bg-brand-900 pt-20 pb-32 text-center overflow-hidden shadow-2xl shadow-brand-900/20 isolate">
            {{-- Abstract Background Elements --}}
            <div
                class="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 rounded-full bg-brand-500 opacity-20 blur-3xl mix-blend-screen">
            </div>
            <div
                class="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 rounded-full bg-brand-400 opacity-10 blur-3xl mix-blend-screen">
            </div>

            {{-- Pattern Overlay (Subtle) --}}
            <div
                class="absolute inset-0 opacity-[0.03] bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
            </div>
            ...
            <div class="relative z-10 max-w-4xl mx-auto px-6">
                ...
                <p class="text-lg md:text-xl text-brand-100/80 max-w-2xl mx-auto font-light leading-relaxed">
                    Mengenal lebih dekat para visioner dan profesional di balik pelestarian arsip Digital Collection
                    Universitas Pakuan.
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
                @foreach ($team as $member)
                    <a href="{{ route('team.show', $member->id) }}" class="block group h-full">
                        <div
                            class="relative h-full bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-xl shadow-gray-200/50 dark:shadow-none transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-900/10">

                            {{-- Badge Logo (Glassmorphism) --}}
                            <div
                                class="absolute top-4 right-4 z-30 flex items-center gap-2 bg-white/30 dark:bg-black/30 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            ...
                            <div class="relative aspect-[3/4] overflow-hidden">
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-brand-900 via-brand-900/40 to-transparent opacity-90 transition-opacity duration-300">
                                </div>
                            </div>
                            ...
                            <div class="absolute bottom-0 inset-x-0 p-6 z-30">
                                ...
                                {{-- Text Content --}}
                                <div
                                    class="relative transform transition-transform duration-300 group-hover:-translate-y-1">
                                    <h3
                                        class="text-2xl font-bold text-white font-['Playfair_Display'] tracking-wide leading-tight mb-1">
                                        {{ $member->name }}
                                    </h3>
                            ...
                            <div
                                class="absolute inset-0 border-2 border-transparent group-hover:border-brand-300/30 rounded-3xl pointer-events-none transition-colors duration-300">
                            </div>
                        </div>
                    </a>
                @endforeach
