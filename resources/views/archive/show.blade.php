        ...
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-3xl border border-gray-100 dark:border-gray-700">
                <div class="p-8">
                    <div class="flex items-center gap-2 mb-8">
                        <span class="w-8 h-8 rounded-full bg-brand-500 text-white flex items-center justify-center text-xs font-bold italic">i</span>
                        <h3 class="text-xl font-black text-gray-800 dark:text-white tracking-tight">{{ __('Metadata Lengkap') }}</h3>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-6">
                            <div>
                                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{{ __('Judul Utama') }}</h4>
                                <p class="text-gray-800 dark:text-gray-200 font-bold">{{ $archive->title }}</p>
                            </div>
                            <div>
                                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{{ __('Pembuat / Creator') }}</h4>
                                <p class="text-gray-800 dark:text-gray-200 font-bold">{{ $archive->creator ?? '-' }}</p>
                            </div>
                            <div>
                                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{{ __('Tanggal Dokumen') }}</h4>
                                <p class="text-gray-800 dark:text-gray-200 font-bold">{{ $archive->date ? \Carbon\Carbon::parse($archive->date)->format('d F Y') : '-' }}</p>
                            </div>
                            <div>
                                <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{{ __('Subjek') }}</h4>
                                <p class="text-gray-800 dark:text-gray-200 font-bold">{{ $archive->subject ?? '-' }}</p>
                            </div>
                        </div>

                        <div class="bg-gray-50/50 dark:bg-gray-900/40 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 grid grid-cols-1 gap-4">
                            <div class="grid grid-cols-2 gap-4 text-xs font-medium">
                                <div>
                                    <span class="block text-gray-400 mb-1">Penerbit</span>
                                    <span class="text-gray-700 dark:text-gray-300">{{ $archive->publisher ?? '-' }}</span>
                                </div>
                                <div>
                                    <span class="block text-gray-400 mb-1">Kontributor</span>
                                    <span class="text-gray-700 dark:text-gray-300">{{ $archive->contributor ?? '-' }}</span>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-700 dark:text-gray-300 font-bold text-brand-500">{{ strtoupper($archive->format ?? 'pdf') }}</span>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-700 dark:text-gray-300">{{ $archive->relation ?? '-' }}</span>
                                </div>
                                <div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
                        <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Deskripsi Singkat') }}</h4>
            ...
            <div class="flex items-center justify-between px-4">
                ...
                <div class="flex gap-3">
                    ...
                    <form action="{{ route('archive.destroy', $archive) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus seluruh arsip dan file di dalamnya?')">
                        @csrf
                        @method('DELETE')
