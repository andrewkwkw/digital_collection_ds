<nav x-data="{ open: false }"
    class="sticky top-0 z-50 backdrop-blur-lg bg-white/70 dark:bg-gray-900/70 border-b border-gray-200/50 dark:border-gray-800/50 transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-2">
                <div
                    class="flex items-center bg-gray-100/50 dark:bg-gray-800/50 p-1 rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
                    <x-theme-toggle />
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-semibold rounded-xl text-gray-600 dark:text-gray-300 hover:text-[#5F8E5F] dark:hover:text-[#5F8E5F] transition-all duration-200 focus:outline-none">
                                <div class="flex items-center">
                                    <div
                                        class="h-8 w-8 rounded-full bg-[#5F8E5F]/10 flex items-center justify-center me-2 border border-[#5F8E5F]/20">
                                        <span
                                            class="text-[#5F8E5F] text-xs font-bold">{{ substr(Auth::user()->name, 0, 1) }}</span>
                                    </div>
                                    {{ Auth::user()->name }}
                                </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open"
                    class="inline-flex items-center justify-center p-2 rounded-xl text-gray-400 hover:text-[#5F8E5F] hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none transition duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        @if(Auth::user()->isSuperAdmin())
            <x-responsive-nav-link :href="route('admin.admin.index')" :active="request()->routeIs('admin.*')">
                {{ __('Daftar Admin') }}
            </x-responsive-nav-link>
